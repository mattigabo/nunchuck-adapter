...
namespace  nunchuckadapter {
    ...
    inline int wiringPiI2CSetup(int deviceAddress) {
        return mockedI2C.i2CFileDescriptor;
    }

    inline void wiringPiI2CWriteReg8(int i2CPortFileDescriptor, int deviceAddress, int data) {
        std::cout <<
    ...
    inline void wiringPiI2CWrite(int i2CPortFileDescriptor, int data) {
        //std::cout << "Write on i2c port > Data: " << data << std::endl;
    }

    inline int wiringPiI2CRead(int i2CPortFileDescriptor) {
        return rand() % 256;
    }
}
